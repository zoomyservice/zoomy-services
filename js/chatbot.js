...
function isFollowUp(n) {
  return anyKw(n, [
    ...
    'what about','and what about','tell me about',
  ]);
}
...
/* ───── CONVERSATIONAL ───── */
{ id:'greeting', kw:['hello','hi','hey','bonjour','salut','hola','buenos','ciao',
...
{ id:'thanks', kw:['thank','thanks','thank you','merci','gracias','cheers','perfect','great',
    'awesome','amazing','wonderful','excellent','parfait','genial','genial'],
...
{ id:'email-marketing', kw:['email marketing','email campaign','newsletter','email automation',
    'email sequence','email list','email template','email broadcast','drip campaign',
    'email funnel','email blast','mailing list','email subscriber',
...
{ id:'contact', kw:['contact','reach','get in touch','write to','talk to','speak to',
    'human','person','team','send a message','contacter','contacto','escribe','correo'],
...
{ id:'quote', kw:['quote','get a quote','pricing','price','pricng','pric','cost','how much','fee','budget',
    'devis','tarif','prix','presupuesto','cuanto','quanto','invoice','bill'],
...
{ id:'freepage', kw:['free','free page','first page','free sample','free website','free build',
    'trial','demo site','gratuit','page gratuite','primera pagina','gratis'],
...
{ id:'campaigns', kw:['campaign','campaigns','paid media','paid ads','advertising',
    'digital advertising','ads management','campagne','campañas','publicidad',
    'social media advertising','run ads','manage ads','run campaigns'],
...
{ id:'google-ads', kw:['google ads','google adwords','adwords','adword','search ads','display ads','youtube ads',
    'ppc','pay per click','search campaign','shopping ads','performance max','pmax',
    'quality score','negative keywords','google shopping'],
...
{ id:'tiktok-ads', kw:['tiktok','tiktok ads','tiktok advertising','tik tok','short video ads',
    'vertical video','reels','tiktok campaign','spark ads','infeed','topfeed'],
...
{ id:'analytics', kw:['analytics','ga4','google analytics','tracking','conversion tracking',
    'gtm','google tag manager','meta pixel','facebook pixel','heatmap','hotjar',
...
{ id:'chatbot', kw:['chatbot','chat bot','website chatbot','chat assistant','chat widget',
    'chat popup','conversational ai','bot','assistant','chat window','live chat',
...
{ id:'websites', kw:['website','site','web','custom site','build site','new site','web design',
    'web development','websit','wesbite','webiste','site web','pagina web','sitio web',
...
{ id:'phoneagent', kw:['phone agent','voice agent','ai phone','phone bot','voice bot','call bot',
    'ai call','answer calls','phone ai','answering service','virtual receptionist',
...
/* ───── CHATBOT ───── */
{ id:'chatbot-training', kw:['how train','training','knowledge base','train chatbot',
...
/* ───── PROCESS & ABOUT ───── */
{ id:'automation', kw:['automation','workflow automation','process automation',
    'zapier','make','integromat','n8n','automate','automated','automate tasks',
    'integrate apps','app integration','connect tools','api integration',
...
{ id:'process', kw:['process','how does it work','how do we start','steps',
    'what happens next','next steps','how to start','getting started','how to begin',
    'processus','comment ça marche','proceso','como funciona','como empezar'],
...
{ id:'seo', kw:['seo','search engine','google ranking','organic search','organic traffic',
    ...
    'rank higher','rank on google','keyword research','appear on google','référencement','posicionamiento'],
  r:() => { ctx.lastEntry='seo';
    return t(
...
function getResponse(userInput) {
  if (!userInput || !userInput.trim()) {
    return t('Please type your question!', 'Tapez votre question !', '¡Escribe tu pregunta!');
  }
  const raw = userInput.trim();
  const n = norm(expandSynonyms(raw));

  // KB scan — first match wins
  for (const entry of KB) {
    if (entry.id === 'fallback') continue;
    if (anyKw(n, entry.kw)) {
      const resp = entry.r();
      memory.push('user', raw, entry.id);
      memory.push('bot', resp, entry.id);
      return resp;
    }
  }
  ...
  if (ctx.lastEntry && FOLLOW_UP[ctx.lastEntry]) {
    ...
    if (fu) {
      memory.push('user', raw, ctx.lastEntry + '-followup');
      memory.push('bot', fu, ctx.lastEntry + '-followup');
      return fu;
    }
  }
  ...
  if (isFollowUp(n) && ctx.lastEntry) {
    const generic = t(
      `Could you be more specific? Try: "How much does the ${ctx.lastEntry} cost?" or "What's included in the ${ctx.lastEntry}?"`,
      `Pouvez-vous être plus précis ? Essayez : "Combien coûte le ${ctx.lastEntry} ?" ou "Qu'est-ce qui est inclus ?"`,
      `¿Podrías ser más específico? Intenta: "¿Cuánto cuesta ${ctx.lastEntry}?" o "¿Qué incluye?"`
    );
    memory.push('user', raw, 'followup');
    memory.push('bot', generic, 'followup');
    return generic;
  }

  // Fallback
  const resp = KB[KB.length - 1].r();
  memory.push('user', raw, 'fallback');
  memory.push('bot', resp, 'fallback');
  return resp;
}
...
if (typeof module !== 'undefined') {
  module.exports = { getResponse, KB, norm, anyKw, kwMatch, expandSynonyms };
}
